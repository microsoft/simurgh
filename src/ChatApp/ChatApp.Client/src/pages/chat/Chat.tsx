...
const Chat = () => {
    ...
    const makeApiRequestWithCosmosDB = async (question: string, conversationId?: string) => {
        ...
        try {
            if (!surveyId) throw Error("surveId cannot be undefined");

            const response = conversationId
                ? await historyGenerate(request, (surveyId as string), abortController.signal, conversationId)
                : await historyGenerate(request, (surveyId as string), abortController.signal)
            if (!response?.ok) {
    ...
    const columns = [
        ...
        //{ key: 'surveyId', name: 'Survey ID', fieldName: 'surveyId', minWidth: 100, maxWidth: 200, isResizable: true },

        { key: 'question', name: 'Question', fieldName: 'question', minWidth: 400, maxWidth: 1000, isResizable: true },
            ...
            {showAuthMessage ? (
                ...
                <Stack horizontal className={styles.chatRoot}>
                    <div className={styles.chatContainer}>
                        {surveyQuestions && (<div className={styles.questionList}><DetailsList
                            compact={true}
                            items={(surveyQuestions as SurveyQuestion[])}
                            columns={columns}
                            setKey="set"
